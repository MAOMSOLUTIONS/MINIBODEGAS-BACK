...
def ocupacion():
    try:
        ...
        datos_completos = {
            'indicadores': datos_agrupados,
            'datos_ocupacion': datos_tabla_agrupados
        }
        ...
        return {
                    ...
                    'body': json.dumps({"message": datos_completos})
...
def insertar_factores():
    ...
    data = [
        {"nombre_factores": "FIJO_TOTAL_RUMBA", "sku_tipo": "RUMBA", "valor_factores": 1932},
        {"nombre_factores": "FIJO_TOTAL_RACK", "sku_tipo": "RACK", "valor_factores": 425},
        {"nombre_factores": "FIJO_FACTOR_DEVOLUCIONES", "sku_tipo": "", "valor_factores": 0.04},
        {"nombre_factores": "FIJO_TOTAL_DEVOLUCIONES", "sku_tipo": "", "valor_factores": 255},
        ...
        {"nombre_factores": "DIAS_INVENTARIO_SEGURIDAD", "sku_tipo": "", "valor_factores": 4},
        {"nombre_factores": "DIAS_TIEMPO_ENTREGA", "sku_tipo": "", "valor_factores": 4},
        {"nombre_factores": "FACTOR_PALLET_M2", "sku_tipo": "", "valor_factores": 1.0625}
...
def obtener_datos_ocupacion():
    try:
        ...
        print("*** DTOS TABLA",lista_datos_ocupacion)
        return {
                    'statusCode': 200,
                    'headers': {
                        'Access-Control-Allow-Origin': 'http://ecinlat.s3-website.us-east-2.amazonaws.com',
                    ...
                    'body': json.dumps({"message": lista_datos_ocupacion})
                }    
    except Exception as e:
        db.session.rollback()
...
def actualizar_datos_ocupacion():
    ...
    factor_pallet_m2 = Factores.query.filter_by(nombre_factores='FACTOR_PALLET_M2').first().valor_factores
...
#                        if datos_ocupacion.area_pronostico_optimo:
        ...
        if ocupacion_rack and factor_fijo_total_rack:
            ocupacion_rack.valor_real = round(((total_area_actualizada_rack / factor_fijo_total_rack)*factor_pallet_m2)*100,1)
            ocupacion_rack.valor_proyectado = round(((total_pronostico_optimo_rack / factor_fijo_total_rack)*factor_pallet_m2)*100,1)
        ...
        if ocupacion_devoluciones and factor_fijo_total_devoluciones:
            ocupacion_devoluciones.valor_real = round(((total_area_actualizada_devoluciones / factor_fijo_total_devoluciones)* factor_pallet_m2) * 100, 1)
        ...
        if not df_sku.empty:
            ...
            if not df_sku.empty:
                ...
                if tipo_modelo=="SARIMA":
                    predicciones_sarima = pronostico_sarima(ventas_suavizadas, dias_pronostico)
                elif tipo_modelo == 'PROPHET':
                    predicciones_sarima = pronostico_prophet(ventas_suavizadas, dias_pronostico)

                print(">>>>>>PREDICCIONES_SARIMA",predicciones_sarima)
...
@api_blueprint.route('/ventas_predicion_xboost_52', methods=['GET'])
def ventas_predicion_xboost_52():
    tipo_modelo = "SARIMA"
    Lambda_valor = 0.3
    factor_valor = 0.8
    ...
    dias_pronostico = int(Factores.query.filter_by(nombre_factores='DIAS_PRONOSTICO').first().valor_factores)
    semanas_pronostico = dias_pronostico/7
    
    # Obtener los datos históricos y de pronóstico
    datos_historicos = db.session.query(
        DatosHistoricos.sku_datos_historicos,
        DatosHistoricos.semana_datos_historicos,
        func.min(DatosHistoricos.fecha_datos_historicos).label('fecha_datos_historicos'),
        func.sum(DatosHistoricos.ventas_datos_historicos).label('ventas_datos_historicos')
    ).filter(
        DatosHistoricos.tipo == 'H'
    ).group_by(
...
#    sku_seleccionado = '2610MD339149'
#    forecast_sku_df = forecast_df.loc[forecast_df['sku_forecats'] == sku_seleccionado]

#    print("AQUI ->>>>",forecast_sku_df)

    skus_sin_historico = []
    nuevas_entradas = []

    # Crear un rango de fechas desde hoy hasta el total de días calculados
    fechas_prediccion = pd.date_range(start=datetime.now().date(), periods=dias_pronostico)
    # Convertir el rango de fechas a DataFrame para facilitar el manejo
    df_fechas = pd.DataFrame(fechas_prediccion, columns=['fecha'])
    # Añadir columna con el número de semana del año para cada fecha
    df_fechas['semana_del_año'] = df_fechas['fecha'].dt.isocalendar().week
    # Agrupar por 'semana_del_año' y obtener la última fecha de cada grupo para identificar las semanas
    ultima_fecha_por_semana = df_fechas.groupby('semana_del_año')['fecha'].max().reset_index()
    ...
    for sku in skus_con_forecast:
        ...
        if not df_sku.empty:
            registro_semana_mas_alta = df_sku.sort_values(by='semana_datos_historicos', ascending=False).iloc[0]
            calculo_historico_ventas = registro_semana_mas_alta["ventas_datos_historicos"]
            for index, row in ultima_fecha_por_semana.iterrows():
                semana = row['semana_del_año']
                fecha_final = row['fecha']
                calculo_pronostico_semanal = (factor_valor * calculo_historico_ventas)
                calculo_historico_ventas = calculo_pronostico_semanal

                    
                if (index<=2):
                    prediccion_no_negativa = max(calculo_pronostico_semanal, 0)
                    prediccion_entero = int(round(prediccion_no_negativa))

                    if (index==1):
                        pronostico_diario = prediccion_entero/7
                    pronostico_optimo+= prediccion_entero

                #construye el pronostico
                nuevo_dato = DatosHistoricos(
                    ano_datos_historicos= "2024",
                    sku_datos_historicos= sku,
                    modelo_datos_historicos= "",
                    tipo_enser_datos_historicos= "",
                    clasificacion_enser_datos_historicos= "",
                    ventas_datos_historicos= calculo_pronostico_semanal,
                    fecha_datos_historicos= fecha_final,
                    semana_datos_historicos= semana,
                    tipo="F"
                )
                nuevas_entradas.append(nuevo_dato)
        try:
            registro = db.session.query(DatosOcupacion).filter_by(sku=sku).first()
            if registro:
                # Actualiza los valores
            ...
            skus_sin_historico.append(df_sku)
    try:
        # Asumiendo que 'db' es tu instancia de SQLAlchemy database.
        db.session.bulk_save_objects(nuevas_entradas)
        registro = db.session.query(DatosOcupacion).filter_by(sku=sku).first()
        ...
        db.session.rollback()
        print(f"Error al insertar los datos: {e}")
    print("skus_sin_historico",skus_sin_historico)    
    # Preparar la respuesta JSON con los datos calculados
    resultados_json = [
        {
            'sku_datos_historicos': dato.sku_datos_historicos,
            'semana_del_ano': dato.semana_datos_historicos,
            'ventas_datos_historicos': dato.ventas_datos_historicos,
            'fecha_datos_historicos': dato.fecha_datos_historicos.strftime("%Y-%m-%d") if isinstance(dato.fecha_datos_historicos, date) else dato.fecha_datos_historicos
        }
        for dato in nuevas_entradas
    ]
    # Enviar respuesta JSON
    return jsonify({
        "mensaje": "Datos insertados con éxito.",
        "datos": resultados_json,
        "skus_sin_historico": len(skus_sin_historico)  # O cualquier otra información relevante sobre SKUs sin histórico
    }), 200
